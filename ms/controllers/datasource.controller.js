...
exports.createDatasource = async (req, res) => {
	...
	try {
		...
		if (cert) {
			newDataSource.connectionDetails.caFilePath = `${
				req.generatedField
			}${path.extname(cert.originalname)}`;
		...
		if (newDataSource.dbType === "bigquery") {
			newDataSource.connectionDetails.serviceKey = newDataSource.keyFileContent;
		}

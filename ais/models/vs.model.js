...
const vectorStoreSchema = new Schema({
	...
	connectionDetails: {
		...
		username: {
			type: String,
			required: () =>
				(this.company == "elasticsearch" && !this.connectionDetails.apiKey) ||
		...
		host: { type: String, required: () => this.company == "pgvector" },
		port: { type: Number, required: () => this.company == "pgvector" },
		pgVectorParams: pgVectorParamsSchema,
		endpoint: { type: String },
		indexName: {
			type: String,
			required: () => this.company == "elasticsearch",
		},
